...
class OutputManager:
    ...
    def get_loading_data(self, design):
        ...
        for i, (tv, d_tb, lv) in enumerate(zip(design.ghe.times, design.ghe.dTb, loading_values)):
            if i + 1 < len(design.ghe.times):
                ...
                wall_temperature = design.ghe.bhe.soil.ugt - d_tb # here I have changed sign
                ...
                if i > 1:
                   ...
                   csv_row.append(0)
                   csv_row.append(0)
                csv_row.append(design.ghe.bhe.soil.ugt - design.ghe.dTb[i - 1]) #I changed sign here
                ...
                current_month = self.hours_to_month(tv)
                normalized_loading = loading / (design.ghe.bhe.b.H * design.ghe.nbh)
                wall_temperature = design.ghe.bhe.soil.ugt - d_tb # I changed sign here
                hp_eft_val = design.ghe.hp_eft[i]
    ...
    def get_summary_object(
        ...
        max_eft_time = design.ghe.times[np.argmax(design.ghe.hp_eft)]
        #min_eft_time = design.ghe.times[design.ghe.hp_eft.index(min(design.ghe.hp_eft))]
        min_eft_time = design.ghe.times[np.argmin(design.ghe.hp_eft)]
